...
class Transformer(nn.Module):
    def __init__(self, input_size, n_heads=8, n_layers=6):
        super(Transformer, self).__init__()

        self.n_layers = n_layers
        self.decoder_attention = nn.ModuleList([TransformerBlock(input_size=input_size, n_heads=n_heads) for _ in range(n_layers)])
            # TransformerBlock(input_size=input_size, n_heads=n_heads)

    def forward(self, Q, K, V, mask=None):
        # for _ in range(self.n_layers):
        #     Q = self.decoder_attention(Q, Q, Q, mask)
        # print("n_layers", self.n_layers)
        for layer in self.decoder_attention:
            Q = layer(Q, Q, Q, mask)
        return Q
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, input_size, d_k=64, d_v=64, n_heads=1, is_layer_norm=True, attn_dropout=0.3):
    ...
    def __init_weights__(self):
        ...
        init.xavier_normal_(self.linear4.weight)
    ...
    def multi_head_attention(self, Q, K, V, mask):
        ...
        if mask is not None:
            ...
            mask = mask.reshape(-1, mask.size(-1))  # bsz n_heads * max_len
    ...
    def forward(self, Q, K, V, mask=None):
        ...
        :param mask: (batch_size, max_words)
        :return:  output: (batch_size, max_q_words, input_size)  same size as Q
        '''

        V_att = self.multi_head_attention(Q, K, V, mask)

        fusion_gate = torch.sigmoid(self.linear3(torch.cat([V_att, Q], dim=-1)))
        fusion = fusion_gate * V_att + (1 - fusion_gate) * Q

        X = self.layer_norm(fusion)

        x_ffn = self.FFN(X)

        fusion_gate1 = torch.sigmoid(self.linear4(torch.cat([x_ffn, X], dim=-1)))
        fusion1 = fusion_gate1 * x_ffn + (1 - fusion_gate1) * X

        output = self.layer_norm(fusion1)

        # V_att = self.multi_head_attention(Q, K, V, mask)
        # X = self.layer_norm(V_att+Q)
        # x_ffn = self.FFN(X)
        # output = self.layer_norm(x_ffn+X)
